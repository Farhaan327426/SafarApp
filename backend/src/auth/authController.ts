...
export async function login(req: Request, res: Response) {
  ...
  try {
    ...
    const csrfToken = req.session ? (req.session as any).csrfToken : null;
...
export async function getSession(req: Request, res: Response) {
  ...
  return res.json({
    ...
    data: {
      user,
      csrfToken: (req.session as any)?.csrfToken || 'active-session-csrf-token'
    },
    requestId
  });
}
...
export async function getCsrfToken(req: Request, res: Response) {
  ...
  const csrfToken = req.session ? (req.session as any).csrfToken : 'safar-csrf-token-2026';
...
function hashOtp(rawOtp: string, phoneNumber: string): string {
  return crypto.createHmac('sha256', OTP_PEPPER).update(`${phoneNumber}:${rawOtp}`).digest('hex');
}
...
export async function verifyOtp(req: Request, res: Response) {
  ...
  if (!crypto.timingSafeEqual(Buffer.from(record.hashedOtp), Buffer.from(candidateHash))) {
    record.attempts += 1;
  ...
  try {
    ...
    if (!user) {
      ...
      const defaultPasswordHash = await argon2.hash(crypto.randomBytes(32).toString('hex'));
