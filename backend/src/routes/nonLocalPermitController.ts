...
const ALLOWED_TRANSITIONS = ['VERIFIED', 'REJECTED', 'SUSPENDED'] as const;
...
export async function verifyNonLocalPermit(req: Request, res: Response) {
  ...
  try {
    ...
    const permit = await prisma.nonLocalPermit.findFirst({
      where: isVehicleReg
...
export async function registerNonLocalPermit(req: Request, res: Response) {
  const requestId = (req as any).requestId;
  const body = req.body;
  ...
  try {
    const permit = await prisma.nonLocalPermit.create({
      data: {
...
export async function approveNonLocalPermit(req: Request, res: Response) {
  const requestId = (req as any).requestId;
  const { id } = req.params;
  const { status } = req.body;

  // Explicit transition allowlist — only VERIFIED, REJECTED, SUSPENDED allowed
  if (!status || !ALLOWED_TRANSITIONS.includes(status as AllowedTransition)) {
    return res.status(400).json({
      success: false,
      error: {
        code: 'INVALID_STATUS',
  ...
  try {
    const permit = await prisma.nonLocalPermit.findUnique({ where: { id } });
    ...
    const updated = await prisma.nonLocalPermit.update({
      where: { id },
...
export async function getNonLocalPermits(req: Request, res: Response) {
  const requestId = (req as any).requestId;
  ...
  if (req.query.home_state) {
    where.homeState = (req.query.home_state as string).toUpperCase();
  }

  if (req.query.verification_status) {
    where.verificationStatus = req.query.verification_status as string;
  }

  if (req.query.entry_border_post) {
    where.entryBorderPost = req.query.entry_border_post as string;
  }

  if (req.query.vehicle_registration) {
    where.vehicleRegistration = {
      contains: (req.query.vehicle_registration as string).toUpperCase(),
      mode: 'insensitive'
    };
  }
  ...
  try {
    const [permits, total] = await Promise.all([
      prisma.nonLocalPermit.findMany({
        where,
        skip,
        take: limit,
      ...
      prisma.nonLocalPermit.count({ where })
    ]);
