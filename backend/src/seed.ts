...
async function seed() {
  ...
  const adminPasswordHash = await argon2.hash('srta@admin2026');
  const auditorPasswordHash = await argon2.hash('auditor@2026');
  ...
  const defaultFare = await prisma.fareVersion.upsert({
  ...
  const source2026 = await prisma.fareSource.upsert({
  ...
  const source2021 = await prisma.fareSource.upsert({
  ...
  for (const rule of initialRules) {
    ...
    if (existing) {
      ...
      await prisma.fareRule.create({ data: rule });
    }
  }
  ...
  for (const src of sroSources) {
    await prisma.fareSource.upsert({
  ...
  const laknaurRoute = await prisma.route.findFirst({
    where: { routeNumber: 'JKMT-01' }
  });
  const banihRoute = await prisma.route.findFirst({
  ...
  const testPermits = [
    {
      ...
      homeState:                    'PB' as const,
      homeStateRegExpiry:           new Date('2027-06-30'),
      vehicleCategory:              'STAGE_CARRIAGE_PERMIT' as const,
    ...
    },
    {
      ...
      homeState:                    'HP' as const,
      homeStateRegExpiry:           new Date('2027-03-31'),
      vehicleCategory:              'CONTRACT_CARRIAGE_PERMIT' as const,
      ...
      taxFeePaidDate:               new Date(),
      verificationStatus:           'PENDING' as const,  // awaits approval
    ...
    },
    {
      ...
      homeState:                    'DL' as const,
      homeStateRegExpiry:           new Date('2026-12-31'),
      vehicleCategory:              'ALL_INDIA_TOURIST_PERMIT' as const,
  ...
  for (const permit of testPermits) {
    await prisma.nonLocalPermit.upsert({
