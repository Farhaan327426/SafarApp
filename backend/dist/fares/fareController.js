...
exports.invalidateFareCache = invalidateFareCache;
exports.getFareQuote = getFareQuote;
exports.getFareSources = getFareSources;
exports.getAdminFares = getAdminFares;
exports.createAdminFare = createAdminFare;
exports.updateAdminFare = updateAdminFare;
exports.verifyAdminFare = verifyAdminFare;
exports.deactivateAdminFare = deactivateAdminFare;
exports.getAuditReport = getAuditReport;
exports.getCurrentFare = getCurrentFare;
exports.getFareHistory = getFareHistory;
exports.saveDraft = saveDraft;
exports.publishFare = publishFare;
exports.rollbackFare = rollbackFare;
exports.getLiveTelemetry = getLiveTelemetry;
exports.updateLiveTelemetry = updateLiveTelemetry;
exports.importGtfs = importGtfs;
exports.getVerifiedFareRules = getVerifiedFareRules;
exports.getActiveSroNotifications = getActiveSroNotifications;
...
async function getFareQuote(req, res) {
    ...
    try {
        ...
        if (routeId && boardingStopId && deboardingStopId) {
            ...
            try {
                const routeObj = await db_1.prisma.route.findUnique({
                    where: { id: String(routeId) },
                    include: { routeStops: { orderBy: { stopSequence: 'asc' } } }
                });
                if (routeObj) {
                    const boardingStop = routeObj.routeStops.find((rs) => rs.stopId === String(boardingStopId));
                    const deboardingStop = routeObj.routeStops.find((rs) => rs.stopId === String(deboardingStopId));
        ...
        let rules = [];
        try {
            rules = await db_1.prisma.fareRule.findMany({
                where: {
                    OR: [
            ...
            rules = SEEDED_FALLBACK_RULES.filter(r => (r.vehicleType === targetVehicleType || r.vehicleCategory === targetCategory) &&
        ...
        const sortedRules = rules.sort((a, b) => {
            ...
            if (aStopMatch !== bStopMatch)
                return bStopMatch - aStopMatch; // 1. Boarding + Deboarding Stop Match
            const aEvent = a.isSpecialEvent ? 1 : 0;
            const bEvent = b.isSpecialEvent ? 1 : 0;
            if (aEvent !== bEvent)
                return bEvent - aEvent; // 2. Special Event Active
            const aRoute = (a.routeId === routeId) ? 1 : 0;
            const bRoute = (b.routeId === routeId) ? 1 : 0;
            if (aRoute !== bRoute)
                return bRoute - aRoute; // 3. Specific Route Match
            ...
            if (aDate !== bDate)
                return bDate - aDate; // Descending: latest date wins
        ...
        if (selectedRule.fareBasis === 'FLAT_FARE' && selectedRule.flatFare !== null && selectedRule.flatFare !== undefined) {
            baseFarePaisa = (0, moneyUtils_1.moneyToPaisa)(Number(selectedRule.flatFare));
            ...
            const firstKmPaisa = (0, moneyUtils_1.moneyToPaisa)(firstKm);
            const subKmPaisa = (0, moneyUtils_1.moneyToPaisa)(subKm);
            if (distanceKm <= 1.0) {
                baseFarePaisa = firstKmPaisa;
            }
            else {
                baseFarePaisa = firstKmPaisa + (0, moneyUtils_1.multiplyPaisa)(subKmPaisa, (distanceKm - 1.0));
            }
            officialFareRate = baseFarePaisa;
            ...
            const perKmPaisa = (0, moneyUtils_1.moneyToPaisa)(perKm);
            baseFarePaisa = (0, moneyUtils_1.multiplyPaisa)(perKmPaisa, distanceKm);
        ...
        if (String(isNight) === 'true') {
            nightHolidaySurchargePaisa = (0, moneyUtils_1.percentagePaisa)(baseFarePaisa, 20);
        }
        // Passenger Category Concession
        let passengerCategoryConcessionPaisa = 0;
        if (String(isConcession) === 'true') {
            passengerCategoryConcessionPaisa = (0, moneyUtils_1.percentagePaisa)(baseFarePaisa, 50); // e.g. 50% off
        }
        let finalFarePaisa = baseFarePaisa + luggageChargesPaisa + nightHolidaySurchargePaisa - passengerCategoryConcessionPaisa;
        // Apply Rounding
        finalFarePaisa = (0, moneyUtils_1.roundPaisa)(finalFarePaisa, 'HALF_UP');
        const responseData = {
            status: 'AVAILABLE',
            fare: {
                amountPaisa: finalFarePaisa,
                amountRupees: (0, moneyUtils_1.paisaToRupees)(finalFarePaisa) // standard canonical format
            },
            fareBreakdown: {
                officialFareRate: (0, moneyUtils_1.paisaToRupees)(officialFareRate),
                maximumPermissibleFare: maximumPermissibleFare ? (0, moneyUtils_1.paisaToRupees)(maximumPermissibleFare) : null,
                routeSpecificFare: routeSpecificFare ? (0, moneyUtils_1.paisaToRupees)(routeSpecificFare) : null,
                minimumFare: minimumFare > 0 ? (0, moneyUtils_1.paisaToRupees)(minimumFare) : null,
                stageFare: stageFare ? (0, moneyUtils_1.paisaToRupees)(stageFare) : null,
                distanceBasedFare: distanceBasedFare ? (0, moneyUtils_1.paisaToRupees)(distanceBasedFare) : null,
                passengerCategoryConcession: passengerCategoryConcessionPaisa > 0 ? (0, moneyUtils_1.paisaToRupees)(passengerCategoryConcessionPaisa) : null,
                luggageCharges: luggageChargesPaisa > 0 ? (0, moneyUtils_1.paisaToRupees)(luggageChargesPaisa) : null,
                specialEventFare: specialEventFarePaisa > 0 ? (0, moneyUtils_1.paisaToRupees)(specialEventFarePaisa) : null,
                nightHolidayCondition: nightHolidaySurchargePaisa > 0 ? (0, moneyUtils_1.paisaToRupees)(nightHolidaySurchargePaisa) : null,
                roundingRules: 'HALF_UP'
            },
            calculatedFare: Number((0, moneyUtils_1.paisaToRupees)(finalFarePaisa)), // backward compatibility
...
async function getFareSources(req, res) {
    const requestId = req.requestId;
    try {
        const sources = await db_1.prisma.fareSource.findMany({
...
async function getAdminFares(req, res) {
    const requestId = req.requestId;
    ...
    try {
        ...
        if (terrain)
            where.terrain = String(terrain);
        const rules = await db_1.prisma.fareRule.findMany({
            where,
...
async function createAdminFare(req, res) {
    const user = req.user;
    const requestId = req.requestId;
    const data = req.body;
    try {
        const newRule = await db_1.prisma.fareRule.create({
            data: {
                ...data,
        ...
        invalidateFareCache();
        await (0, auditLogger_1.logAuditEvent)(user.id, 'FARE_RULE_CREATED', 'FARE_RULE', newRule.id, { vehicleType: newRule.vehicleType }, requestId);
...
async function updateAdminFare(req, res) {
    const user = req.user;
    const requestId = req.requestId;
    const { id } = req.params;
    const data = req.body;
    try {
        ...
        const updatedRule = await db_1.prisma.fareRule.update({
            where: { id },
            data: {
                ...data,
        ...
        invalidateFareCache();
        await (0, auditLogger_1.logAuditEvent)(user.id, 'FARE_RULE_UPDATED', 'FARE_RULE', id, { status: 'REVIEW_REQUIRED' }, requestId);
        return res.json({
            success: true,
            data: updatedRule,
            requestId
...
async function verifyAdminFare(req, res) {
    const user = req.user;
    const requestId = req.requestId;
    const { id } = req.params;
    try {
        const verifiedRule = await db_1.prisma.fareRule.update({
            where: { id },
            data: {
                verificationStatus: 'VERIFIED',
                updatedAt: new Date()
        ...
        invalidateFareCache();
        await (0, auditLogger_1.logAuditEvent)(user.id, 'FARE_RULE_VERIFIED', 'FARE_RULE', id, { status: 'VERIFIED' }, requestId);
        return res.json({
            success: true,
            data: verifiedRule,
            requestId
...
async function deactivateAdminFare(req, res) {
    const user = req.user;
    const requestId = req.requestId;
    const { id } = req.params;
    try {
        const deactivatedRule = await db_1.prisma.fareRule.update({
            where: { id },
            data: {
        ...
        invalidateFareCache();
        await (0, auditLogger_1.logAuditEvent)(user.id, 'FARE_RULE_DEACTIVATED', 'FARE_RULE', id, { status: 'DEACTIVATED' }, requestId);
        return res.json({
            success: true,
...
async function getAuditReport(req, res) {
    ...
    try {
        let allRules = [];
        try {
            allRules = await db_1.prisma.fareRule.findMany();
        }
        catch (err) {
        ...
        const overlappingValidityPeriods = [];
...
async function getCurrentFare(req, res) {
    const requestId = req.requestId;
    try {
        const publishedFare = await db_1.prisma.fareVersion.findFirst({
...
async function getFareHistory(req, res) {
    const requestId = req.requestId;
    try {
        const fares = await db_1.prisma.fareVersion.findMany({
            orderBy: { version: 'desc' }
        });
        return res.json({
            success: true,
...
async function saveDraft(req, res) {
    ...
    try {
        let draft = await db_1.prisma.fareVersion.findFirst({
            where: { status: 'DRAFT' }
        });
        if (draft) {
            draft = await db_1.prisma.fareVersion.update({
                where: { id: draft.id },
                data: {
                    rates,
                    slabs,
            ...
            const lastVersion = await db_1.prisma.fareVersion.findFirst({
                orderBy: { version: 'desc' }
            });
            const nextVersion = (lastVersion?.version || 20260800) + 1;
            draft = await db_1.prisma.fareVersion.create({
                data: {
                    version: nextVersion,
                    status: 'DRAFT',
                    rates,
        ...
        await (0, auditLogger_1.logAuditEvent)(user.id, 'FARE_DRAFT_CREATED', 'FARE_VERSION', draft.id, { version: draft.version }, requestId);
        return res.json({
            success: true,
            data: draft,
            requestId
...
async function publishFare(req, res) {
    const user = req.user;
    const requestId = req.requestId;
    const { rates, slabs, vehicleMultipliers, expectedBaseVersion, reason } = req.body;
    try {
        const currentActive = await db_1.prisma.fareVersion.findFirst({
            where: { status: 'PUBLISHED' },
            orderBy: { version: 'desc' }
        });
        ...
        if (currentActive) {
            await db_1.prisma.fareVersion.update({
                where: { id: currentActive.id },
                data: { status: 'SUPERSEDED' }
            });
        }
        const nextVersion = (currentActive?.version || 20260800) + 1;
        const publishedFare = await db_1.prisma.fareVersion.create({
            data: {
                version: nextVersion,
                status: 'PUBLISHED',
                rates,
        ...
        invalidateFareCache();
        await (0, auditLogger_1.logAuditEvent)(user.id, 'FARE_PUBLISHED', 'FARE_VERSION', publishedFare.id, { version: publishedFare.version }, requestId);
        return res.json({
            success: true,
            data: publishedFare,
            requestId
...
async function rollbackFare(req, res) {
    const user = req.user;
    const requestId = req.requestId;
    try {
        const currentActive = await db_1.prisma.fareVersion.findFirst({
        ...
        const previousSuperseded = await db_1.prisma.fareVersion.findFirst({
        ...
        const restoredFare = await db_1.prisma.fareVersion.update({
        ...
        invalidateFareCache();
        await (0, auditLogger_1.logAuditEvent)(user.id, 'FARE_ROLLED_BACK', 'FARE_VERSION', restoredFare.id, { restoredVersion: restoredFare.version }, requestId);
        return res.json({
            success: true,
            data: restoredFare,
            requestId
...
async function getLiveTelemetry(req, res) {
...
async function updateLiveTelemetry(req, res) {
    const user = req.user;
    const requestId = req.requestId;
...
async function getVerifiedFareRules(req, res) {
    const requestId = req.requestId;
    try {
        const rules = await db_1.prisma.fareRule.findMany({
...
async function getActiveSroNotifications(req, res) {
    const requestId = req.requestId;
    try {
        const notifications = await db_1.prisma.fareSource.findMany({
            where: {
                isActiveSro: true,
                verificationStatus: 'VERIFIED'
            },
